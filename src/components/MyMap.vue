<template>
  <pre v-if="$selectedRecord">{{ $selectedRecord.id }}</pre>
  <pre>Zoom: {{zoom}}</pre>
  <pre>Center: {{center}}</pre>
    <l-map id="map" ref="map" v-model:zoom="zoom" :center="center" zoomDelta=0.5 :maxBounds="[[40.522151, -74.381561],[40.951900, -73.26438]]" :minZoom=9 :options="{zoomControl: false}">
      ...
      <l-marker v-for="record in remote_data.records" :key="record.id" :lat-lng="[record.fields.lat, record.fields.lng]" @click="setSelected(record)">
        <l-popup>
  ...
  const zoom = ref(12)
  const center = ref([40.619888, -73.917170])
  ...
  function setSelected(record) {
    ...
    moveToLocation(record)
  }

  function moveToLocation(record){
    const latLng = [parseFloat(record.fields.lat), parseFloat(record.fields.lng)]
    zoom.value = 14
    center.value = latLng
